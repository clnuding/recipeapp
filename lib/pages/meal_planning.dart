...
class MealPlanningPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 3, vertical: 10), // Equal left & right padding
          child: Column(
            children: [
              // Page Title
              Text(
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        double totalHeight = constraints.maxHeight; // Ensure table fills the screen
        ...
        return Container(
          ...
          child: ClipRRect(
            ...
            child: DataTable(
              ...
              dataRowHeight: (totalHeight - 45) / mealData.length, // Even row height for full screen usage
              headingRowColor: MaterialStateProperty.all(theme.alternateColor), // Light grey header row
              dataRowColor: MaterialStateProperty.resolveWith<Color?>((Set<MaterialState> states) {
                return states.contains(MaterialState.selected) ? const Color.fromARGB(255, 43, 42, 42) : null;
              }),
              border: TableBorder.all(color: theme.primaryText.withOpacity(0.4), width: 0.5),
              columns: [
                DataColumn(
                  label: SizedBox(
                    width: dayColumnWidth,
                    child: Text('Day', style: theme.bodyMedium, softWrap: true),
                  ),
                ),
                DataColumn(
                  label: SizedBox(
                    width: mealColumnWidth,
                    child: Text('Breakfast', style: theme.bodyMedium, softWrap: true),
                  ),
                ),
                DataColumn(
                  label: SizedBox(
                    width: mealColumnWidth,
                    child: Text('Lunch', style: theme.bodyMedium, softWrap: true),
                  ),
                ),
                DataColumn(
                  label: SizedBox(
                    width: mealColumnWidth,
                    child: Text('Dinner', style: theme.bodyMedium, softWrap: true),
                  ),
                ),
              ],
              rows: mealData.map((meal) {
                return DataRow(
                  color: MaterialStateProperty.all(theme.primaryBackground.withOpacity(0.05)),
                  cells: [
                    DataCell(
                      SizedBox(
                        width: dayColumnWidth,
                        child: Text(meal["day"] ?? '', style: theme.bodySmall, softWrap: true),
                      ),
                    ),
                    DataCell(
                      SizedBox(
                        width: mealColumnWidth,
                        child: Text(meal["breakfast"] ?? '', style: theme.bodySmall, softWrap: true),
                      ),
                    ),
                    DataCell(
                      SizedBox(
                        width: mealColumnWidth,
                        child: Text(meal["lunch"] ?? '', style: theme.bodySmall, softWrap: true),
                      ),
                    ),
                    DataCell(
                      SizedBox(
                        width: mealColumnWidth,
                        child: Text(meal["dinner"] ?? '', style: theme.bodySmall, softWrap: true),
                      ),
                    ),
                  ],
                );
